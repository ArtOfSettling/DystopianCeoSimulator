...
pub struct FanOutClientCommandSender {
    pub(crate) tx_fan_out_client_commands: Sender<(Uuid, ClientCommand)>,
}

#[derive(Resource)]
pub struct FanOutClientCommandReceiver {
    pub(crate) rx_fan_out_client_commands: Receiver<(Uuid, ClientCommand)>,
}
...
pub fn setup_fan_out_commands(mut commands: Commands) {
    ...
    commands.insert_resource(FanOutClientCommandSender {
        tx_fan_out_client_commands: tx_client_commands,
    });
    commands.insert_resource(FanOutClientCommandReceiver {
        rx_fan_out_client_commands: rx_client_commands,
    });
